...
当前版本：`v0.1.0`。版本记录见 [更新日志](CHANGELOG.md)。

文档：[技术与实现方案](docs/implementation.md) · [仓库目录与文件管理](docs/repository-layout.md) · [贡献指南](CONTRIBUTING.md)。
...
| `scripts/blender/build_scenes.py` | 可编辑的 Blender 程序化场景源码 |
| `models/*.blend` | 可编辑的场景母版 |
| `scripts/blender/render_delivery.py` | 室外场景静态图和城市至公寓楼的镜头动画 |
| `scripts/assets/export_assets.mjs` | Web 媒体优化与导出 |

网站使用本地资源，不连接真实设备，也没有后端实时数据。开关柜画面和应用文案仅用于概念展示，不代表具体产品或工程图纸。

### 重建媒体资源

运行网站所需的优化媒体已包含在 `web/public/assets/`。可使用 Blender 5.2 和 `scripts/blender/build_scenes.py` 重建可编辑资源；完整电气室全景图的渲染时间明显长于静态图。仓库包含场景母版，无需获取参考网站的模型或媒体即可编辑。
...
Current version: `v0.1.0`. See the [changelog](CHANGELOG.md) for release notes.

Documentation (Chinese): [Implementation guide](docs/implementation.md) · [Repository layout](docs/repository-layout.md) · [Contributing](CONTRIBUTING.md).
...
| Path | Purpose |
| --- | --- |
| `docs/` | Architecture, repository layout, and maintenance documentation |
...
| `scripts/blender/build_scenes.py` | Editable procedural Blender scene source |
| `models/*.blend` | Editable scene masters |
| `scripts/blender/render_delivery.py` | Exterior stills and city-to-apartment camera move |
| `scripts/assets/export_assets.mjs` | Optimized web media export |
...
The optimized media required to run the site is already under `web/public/assets/`. Blender 5.2 can rebuild the editable assets from `scripts/blender/build_scenes.py`; the complete room panorama takes substantially longer than the still images. Source masters are included so the scenes can be edited without acquiring the reference site's models or media.
