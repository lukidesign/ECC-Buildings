...
for(const name of ['city','apartment','room','product','room-panorama']) {
  ...
  const input=path.join(root,'.local/renders',source+'.png');
  ...
  manifest.push({name,source:`.local/renders/${source}.png`,output:`web/public/assets/${name}.webp`,width:result.width,height:result.height,bytes:result.size,origin:'Independent Blender model and Cycles render'});
  if(['city','apartment','room'].includes(name)) {
    const meta=preview&&name==='apartment'?'city-apartment':name;
    await copyFile(path.join(root,'.local/renders',meta+'-metadata.json'),path.join(out,name+'-metadata.json'));
  }
}
